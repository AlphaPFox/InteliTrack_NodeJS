...
class TK102B extends Tracker
{
	...
	//Called when tracker connects to server or send a new message
	setConnection(socket)
	{
		//Save connection socket
		this._socket = socket;

		//Save tracker reference on this connnection
		this._socket.onDisconnect = this.closeConnection.bind(this);
	}
   ...
   getConnection()
   {
	...
	//Called when tracker disconnects
	closeConnection()
	{
	...
	sendCommand(command)
   {
		  ...
		  else if(type === "tcp_data" && data[1])
		  {
				...
				if(data.length > 10)
				{
					...
					if(data[4] == "F")
					{
						...
						var datetime = moment.utc(data[2].substring(0, 6) + data[5].substring(0, 6), "YYMMDDhhmmss").toDate();
					...
					else
					{
						...
						{  
							...
							{
								//Parse datetime (ex.: 181106115734)
								var datetime = moment(data[2], "YYMMDDhhmmss").toDate();
    ...
    insert_coordinates(tracker_params, coordinate_params, text)
    {
        ...
        else if(text.includes("overspeed"))
        {
